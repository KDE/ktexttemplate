...
void TestInternationalization::testStrings_data()
{
    ...
    QTest::newRow("string-02") << "%n People"
                               << "1 People"
                               << "1 Person"
                               << "1 Personne" << QString() << QStringLiteral("%n People") << QVariantList{1};

    QTest::newRow("string-03") << "%n People"
                               << "2 People"
                               << "2 Personen"
                               << "2 Personnes" << QString() << QStringLiteral("%n People") << QVariantList{2};

    QTest::newRow("string-04") << "Name"
                               << "Name"
                               << "Name eines Buches"
                               << "Nom d'un livre" << QStringLiteral("Name of a Book") << QString() << QVariantList();

    QTest::newRow("string-05") << "Name"
                               << "Name"
                               << "Namen einer Person"
                               << "Nom d'une personne" << QStringLiteral("Name of a Person") << QString() << QVariantList();

    QTest::newRow("string-06") << "%n People"
                               << "1 People"
                               << "1 Person angemeldet" << QStringLiteral("1 Personne connecté") << QStringLiteral("%n people are logged in")
                               << QStringLiteral("%n People") << QVariantList{1};

    QTest::newRow("string-07") << "%n People"
                               << "2 People"
                               << "2 Personen angemeldet" << QStringLiteral("2 Personnes connecté") << QStringLiteral("%n people are logged in")
                               << QStringLiteral("%n People") << QVariantList{2};

    QTest::newRow("string-08") << "%n file(s) copied to %1"
                               << "1 files copied to destinationFolder"
                               << "1 Datei in destinationFolder kopiert" << QStringLiteral("1 fichier copié dans destinationFolder") << QString()
                               << QStringLiteral("%n files copied to %1") << QVariantList{1, QStringLiteral("destinationFolder")};

    QTest::newRow("string-09") << "%n file(s) copied to %1"
                               << "2 files copied to destinationFolder"
                               << "2 Datein in destinationFolder kopiert" << QStringLiteral("2 fichiers copiés dans destinationFolder") << QString()
                               << QStringLiteral("%n files copied to %1") << QVariantList{2, QStringLiteral("destinationFolder")};

    QTest::newRow("string-10") << "%n to %1"
                               << "1 copied to destinationFolder"
                               << "1 Datei wird nach destinationFolder kopiert" << QStringLiteral("1 fichier est copié sur destinationFolder")
                               << QStringLiteral("Files are being copied") << QStringLiteral("%n copied to %1")
                               << QVariantList{1, QStringLiteral("destinationFolder")};

    QTest::newRow("string-11") << "%n to %1"
                               << "1 copied to destinationFolder"
                               << "1 Datei war nach destinationFolder kopiert" << QStringLiteral("1 fichier a été copié à destinationFolder")
                               << QStringLiteral("Files have already been copied") << QStringLiteral("%n copied to %1")
                               << QVariantList{1, QStringLiteral("destinationFolder")};

    QTest::newRow("string-12") << "%n to %1"
                               << "2 copied to destinationFolder"
                               << "2 Datein wird nach destinationFolder kopiert" << QStringLiteral("2 fichiers sont copiés à destinationFolder")
                               << QStringLiteral("Files are being copied") << QStringLiteral("%n copied to %1")
                               << QVariantList{2, QStringLiteral("destinationFolder")};

    QTest::newRow("string-13") << "%n to %1"
                               << "2 copied to destinationFolder"
                               << "2 Datein war nach destinationFolder kopiert" << QStringLiteral("2 fichiers ont été copiés sur destinationFolder")
                               << QStringLiteral("Files have already been copied") << QStringLiteral("%n copied to %1")
                               << QVariantList{2, QStringLiteral("destinationFolder")};

    QTest::newRow("string-14") << "from %1 to %2"
                               << "from sourceFolder to destinationFolder"
                               << "nach destinationFolder von sourceFolder" << QStringLiteral("à partir de sourceFolder destinationFolder")
    ...
    QTest::newRow("string-15") << "%1 messages at %2, fraction of total: %3. Rating : %4"
                               ...
                               << QStringLiteral(
                                      ...
                                      "0.6. Note: 4.8")
                               << QString() << QString() << QVariantList{1000, QDate(2005, 5, 7), 0.6, 4.8};

#ifndef Q_CC_MSVC
    QTest::newRow("string-16") << "Snowman: ☃"
                               << "Snowman: ☃"
                               << "Schneemann: ☃" << QStringLiteral("bonhomme de neige: ☃") << QString() << QString() << QVariantList();
#endif
}
...
void TestInternationalization::testLocalizedTemplate_data()
{
    ...
    QTest::newRow("fragment-04") << QString::fromLatin1(
    ...
    QTest::newRow("fragment-05") << QString::fromLatin1(
        "{{ _('Today') }} -- {{ _(integer) }} -- {{ _(date) }} -- {{ "
        "_(smallFloat) }} -- {{ _(largeFloat) }} -- {{ _(time) }} -- {{ "
        "_(dateTime) }}"
        "{% with_locale 'de_DE' %}"
    ...
    QTest::newRow("fragment-06") << QStringLiteral("{{ _(list.0) }}") << QStringLiteral("1000") << QStringLiteral("1,000") << QStringLiteral("1,000")
                                 << QStringLiteral("1.000") << QString::fromUtf8("1" FR_THOUSAND_SEPARATOR "000") << dict;
    ...
    QTest::newRow("fragment-07") << QStringLiteral("{{ _(longlong) }} {{ _(float) }} {{ _(double) }}{{ _(hash) }}") << QStringLiteral("1000 0.60 4.80")
                                 << QStringLiteral("1,000 0.60 4.80") << QStringLiteral("1,000 0.60 4.80") << QStringLiteral("1.000 0,60 4,80")
                                 << QString::fromUtf8("1" FR_THOUSAND_SEPARATOR "000 0,60 4,80") << dict;

    QTest::newRow("fragment-08") << QStringLiteral("{{ 'this'|cut:_(\"i\") }}") << QStringLiteral("ths") << QStringLiteral("ths") << QStringLiteral("ths")
                                 << QStringLiteral("ths") << QStringLiteral("ths") << dict;
    ...
    QTest::newRow("fragment-09") << QStringLiteral("{% l10n_filesize fs_int_mib %}") << QStringLiteral("1.05 MB") << QStringLiteral("1.05 MB")
                                 << QStringLiteral("1.05 MB") << QStringLiteral("1,05 MB") << QStringLiteral("1,05 Mo") << dict;

    QTest::newRow("fragment-10") << QStringLiteral("{% l10n_filesize_var fs_int_mib size_var %}{{ size_var }}") << QStringLiteral("1.05 MB")
                                 << QStringLiteral("1.05 MB") << QStringLiteral("1.05 MB") << QStringLiteral("1,05 MB") << QStringLiteral("1,05 Mo") << dict;

    QTest::newRow("fragment-11") << QStringLiteral("{% l10n_filesize fs_int_mib 2 %}") << QStringLiteral("1.00 MiB") << QStringLiteral("1.00 MiB")
                                 << QStringLiteral("1.00 MiB") << QStringLiteral("1,00 MiB") << QStringLiteral("1,00 Mio") << dict;

    QTest::newRow("fragment-12") << QStringLiteral("{% l10n_filesize_var fs_int_mib 2 size_var %}{{ size_var }}") << QStringLiteral("1.00 MiB")
                                 << QStringLiteral("1.00 MiB") << QStringLiteral("1.00 MiB") << QStringLiteral("1,00 MiB") << QStringLiteral("1,00 Mio")
                                 << dict;

    QTest::newRow("fragment-13") << QStringLiteral("{% l10n_filesize fs_int_mib 10 3 %}") << QStringLiteral("1.049 MB") << QStringLiteral("1.049 MB")
                                 << QStringLiteral("1.049 MB") << QStringLiteral("1,049 MB") << QStringLiteral("1,049 Mo") << dict;

    QTest::newRow("fragment-14") << QStringLiteral("{% l10n_filesize_var fs_int_mib 10 3 size_var %}{{ size_var }}") << QStringLiteral("1.049 MB")
                                 << QStringLiteral("1.049 MB") << QStringLiteral("1.049 MB") << QStringLiteral("1,049 MB") << QStringLiteral("1,049 Mo")
                                 << dict;

    QTest::newRow("fragment-15") << QStringLiteral("{% l10n_filesize fs_int_mib 10 2 1024 %}") << QStringLiteral("1.07 GB") << QStringLiteral("1.07 GB")
                                 << QStringLiteral("1.07 GB") << QStringLiteral("1,07 GB") << QStringLiteral("1,07 Go") << dict;

    QTest::newRow("fragment-16") << QStringLiteral("{% l10n_filesize_var fs_int_mib 10 2 1024 size_var %}{{ size_var }}") << QStringLiteral("1.07 GB")
                                 << QStringLiteral("1.07 GB") << QStringLiteral("1.07 GB") << QStringLiteral("1,07 GB") << QStringLiteral("1,07 Go") << dict;

    dict.clear();
    dict.insert(QStringLiteral("fs_float_mib"), 1024.5);

    QTest::newRow("fragment-17") << QStringLiteral("{% l10n_filesize fs_float_mib 10 2 1024 %}") << QStringLiteral("1.05 MB") << QStringLiteral("1.05 MB")
                                 << QStringLiteral("1.05 MB") << QStringLiteral("1,05 MB") << QStringLiteral("1,05 Mo") << dict;

    QTest::newRow("fragment-18") << QStringLiteral("{% l10n_filesize_var fs_float_mib 10 2 1024 size_var %}{{ size_var }}") << QStringLiteral("1.05 MB")
                                 << QStringLiteral("1.05 MB") << QStringLiteral("1.05 MB") << QStringLiteral("1,05 MB") << QStringLiteral("1,05 Mo") << dict;

    dict.clear();
    dict.insert(QStringLiteral("fs_string_mib"), QStringLiteral("1024.5"));

    QTest::newRow("fragment-19") << QStringLiteral("{% l10n_filesize fs_string_mib 10 2 1024 %}") << QStringLiteral("1.05 MB") << QStringLiteral("1.05 MB")
                                 << QStringLiteral("1.05 MB") << QStringLiteral("1,05 MB") << QStringLiteral("1,05 Mo") << dict;

    QTest::newRow("fragment-20") << QStringLiteral("{% l10n_filesize_var fs_string_mib 10 2 1024 size_var %}{{ size_var }}") << QStringLiteral("1.05 MB")
                                 << QStringLiteral("1.05 MB") << QStringLiteral("1.05 MB") << QStringLiteral("1,05 MB") << QStringLiteral("1,05 Mo") << dict;
}
...
void TestInternationalization::testSafeContent_data()
{
    ...
    QTest::newRow("safe-07") << QStringLiteral("-{{ _(today_text) }}-") << QStringLiteral("-Today-") << QStringLiteral("-Aujourd&#39;hui-") << dict;

    QTest::newRow("safe-08") << QStringLiteral("{% autoescape off %}-{{ _(today_text) }}-{% endautoescape %}") << QStringLiteral("-Today-")
                             << QStringLiteral("-Aujourd'hui-") << dict;

    dict.insert(QStringLiteral("today_and_tomorrow_text"), markSafe(QStringLiteral("Today &amp; tomorrow")));

    QTest::newRow("safe-09") << QStringLiteral("-{{ _(today_and_tomorrow_text) }}-") << QStringLiteral("-Today &amp;amp; tomorrow-")
    ...
    QTest::newRow("safe-11") << QStringLiteral("{% i18nc 'The current day is' 'Today is' %}") << QStringLiteral("Today is")
    ...
    QTest::newRow("safe-14") << QStringLiteral(
        ...
        "today_greeting %}-{{ today_greeting }}-{% endautoescape %}")
                             << QStringLiteral("-Today is-") << QStringLiteral("-Aujourd'hui est-") << dict;

    QTest::newRow("safe-15") << QStringLiteral("{% i18np '%n people today' 1 %}") << QStringLiteral("1 people today") // Not really testing English here.
                             << QStringLiteral("1 personne aujourd&#39;hui") << dict;

    QTest::newRow("safe-16") << QStringLiteral("{% i18np '%n people today' 2 %}") << QStringLiteral("2 people today")
                             << QStringLiteral("2 personnes aujourd&#39;hui") << dict;

    QTest::newRow("safe-17") << QStringLiteral("{% autoescape off %}{% i18np '%n people today' 1 %}{% endautoescape %}")
                             << QStringLiteral("1 people today") // Not really testing
                                                                 // English here.
                             << QStringLiteral("1 personne aujourd'hui") << dict;

    QTest::newRow("safe-18") << QStringLiteral("{% autoescape off %}{% i18np '%n people today' 2 %}{% endautoescape %}") << QStringLiteral("2 people today")
                             << QStringLiteral("2 personnes aujourd'hui") << dict;

    QTest::newRow("safe-19") << QStringLiteral("{% i18np_var '%n people today' 1 as num_people %}-{{ num_people }}-")
                             << QStringLiteral("-1 people today-") // Not really testing
                                                                   // English here.
                             << QStringLiteral("-1 personne aujourd&#39;hui-") << dict;

    QTest::newRow("safe-20") << QStringLiteral("{% i18np_var '%n people today' 2 as num_people %}-{{ num_people }}-") << QStringLiteral("-2 people today-")
                             << QStringLiteral("-2 personnes aujourd&#39;hui-") << dict;

    QTest::newRow("safe-21") << QStringLiteral(
    ...
    QTest::newRow("safe-22") << QStringLiteral(
        ...
        "%}-{{ num_people }}-{% endautoescape %}")
                             << QStringLiteral("-2 people today-") << QStringLiteral("-2 personnes aujourd'hui-") << dict;

    QTest::newRow("safe-23") << QStringLiteral(
    ...
    QTest::newRow("safe-24") << QStringLiteral(
        "{% i18ncp 'The number of people who have visited "
        "today' '%n people visited today' 2 %}")
                             << QStringLiteral("2 people visited today") << QStringLiteral("2 personnes a visité aujourd&#39;hui") << dict;

    QTest::newRow("safe-25") << QStringLiteral(
    ...
    QTest::newRow("safe-27") << QStringLiteral(
    ...
    QTest::newRow("safe-29") << QStringLiteral(
...
void TestInternationalization::testFailure_data()
{
    ...
    QTest::newRow("fail-01") << QStringLiteral("{% i18np_var '%n people visited today' as num_people %}");
    QTest::newRow("fail-02") << QStringLiteral(
        "{% i18ncp_var 'The number of people who have visited today' '%n people "
        "visited today' as num_people %}");
    QTest::newRow("fail-03") << QStringLiteral("{% i18np '%n people visited today' %}");
...
void TestInternationalization::testIntegers_data()
{
    ...
    QTest::newRow("integer-01") << 7 << QStringLiteral("7") << QStringLiteral("7") << QStringLiteral("7") << QStringLiteral("7") << QStringLiteral("7")
                                << QStringLiteral("7");

    QTest::newRow("integer-02") << 7000 << QStringLiteral("7000") << QStringLiteral("7000") << QStringLiteral("7,000") << QStringLiteral("7,000")
                                << QStringLiteral("7.000") << QString::fromUtf8("7" FR_THOUSAND_SEPARATOR "000");
}
...
void TestInternationalization::testFloats_data()
{
    ...
    QTest::newRow("float-01") << qreal(7.4) << QStringLiteral("7.4") << QStringLiteral("7.40") << QStringLiteral("7.40") << QStringLiteral("7.40")
