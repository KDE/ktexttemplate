...
void BookWindow::renderBooks() const
{
    ...
    if (!t) {
        QMessageBox::critical(const_cast<BookWindow *>(this), "Unable to load template", QString("Error loading template: %1").arg(themeName + ".html"));
        return;
    }

    if (t->error()) {
        QMessageBox::critical(const_cast<BookWindow *>(this), "Unable to load template", QString("Error loading template: %1").arg(t->errorString()));
        return;
    }

    bool ok;
    QString text = QInputDialog::getText(const_cast<BookWindow *>(this),
                                         tr("Export Location"),
                                         tr("file name:"),
                                         QLineEdit::Normal,
                                         QDir::home().absolutePath() + "/book_export.html",
                                         &ok);
    if (!ok || text.isEmpty())
        return;
    ...
    if (t->error()) {
        QMessageBox::critical(const_cast<BookWindow *>(this), "Unable render template", QString("Error rendering template: %1").arg(t->errorString()));
        return;
    }
