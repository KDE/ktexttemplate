...
void DesignWidget::insertMethod(const QString &access, bool _virtual, const QString &type, const QString &name, bool _const, QList<QStringList> args)
{
    ...
    method << new QStandardItem(access) << new QStandardItem() << new QStandardItem(type) << new QStandardItem(name) << new QStandardItem();
    ...
    if (!args.isEmpty()) {
        for (const QStringList &arg : args) {
            ...
            argItem << new QStandardItem(arg.at(0)) << new QStandardItem(arg.at(1)) << new QStandardItem(arg.value(2));
