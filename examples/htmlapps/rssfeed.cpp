...
KTextTemplate::Node *RssFeedNodeFactory::getNode(const QString &tagContent,
...
RssFeedNode::RssFeedNode(const KTextTemplate::FilterExpression &url,
                         const KTextTemplate::FilterExpression &query,
                         QObject *parent)
    : KTextTemplate::Node(parent), m_url(url), m_query(query)
{
}
...
void RssFeedNode::render(KTextTemplate::OutputStream *stream,
                         KTextTemplate::Context *c) const
{
  ...
  query.setQuery(ns + "doc($inputDocument)"
                 + KTextTemplate::getSafeString(m_query.resolve(c)).get());
...
KTextTemplate::Node *XmlRoleNodeFactory::getNode(const QString &tagContent,
                                            KTextTemplate::Parser *p) const
{
  QStringList expr = smartSplit(tagContent);
...
XmlRoleNode::XmlRoleNode(const KTextTemplate::FilterExpression &query,
                         QObject *parent)
...
void XmlRoleNode::render(KTextTemplate::OutputStream *stream,
                         KTextTemplate::Context *c) const
{
...
KTextTemplate::Node *XmlNamespaceNodeFactory::getNode(const QString &tagContent,
                                                 KTextTemplate::Parser *p) const
{
  QStringList expr = smartSplit(tagContent);
  KTextTemplate::FilterExpression query(expr.at(1), p);
...
XmlNamespaceNode::XmlNamespaceNode(const KTextTemplate::FilterExpression &query,
                                   const QString &name, QObject *parent)
    : m_query(query), m_name(name)
{
}

void XmlNamespaceNode::render(KTextTemplate::OutputStream *stream,
                              KTextTemplate::Context *c) const
{
