...
static QList<std::pair<QString, QString>> getJsEscapes()
{
    ...
    jsEscapes << std::pair<QString, QString>(QChar::fromLatin1('\\'), QStringLiteral("\\u005C"))
              ...
              << std::pair<QString, QString>(QChar::fromLatin1('-'), QStringLiteral("\\u002D"))
              << std::pair<QString, QString>(QChar::fromLatin1(';'), QStringLiteral("\\u003B"))
              << std::pair<QString, QString>(QChar(0x2028), QStringLiteral("\\u2028")) << std::pair<QString, QString>(QChar(0x2029), QStringLiteral("\\u2029"));
...
QVariant LineNumbersFilter::doFilter(const QVariant &input, const QVariant &argument, bool autoescape) const
{
    ...
    for (auto i = 0; i < lines.size(); ++i) {
        lines[i] = QStringLiteral("%1. %2").arg(i + 1, width).arg(shouldEscape ? QString(escape(lines.at(i))) : lines.at(i));
    }
...
QVariant RemoveTagsFilter::doFilter(const QVariant &input, const QVariant &argument, bool autoescape) const
{
    ...
    const auto tagRe = QStringLiteral("(%1)").arg(tags.join(QChar::fromLatin1('|')));
...
QVariant WordWrapFilter::doFilter(const QVariant &input, const QVariant &argument, bool autoescape) const
{
    ...
    for (const QString &part : std::as_const(partList)) {
        QStringList lines;
...
QVariant FileSizeFormatFilter::doFilter(const QVariant &input, const QVariant &argument, bool autoescape) const
{
    QVariant ret;
    ...
    if (!arg.get().isEmpty()) {
        ...
        if (numArgs > 0) {
            ...
            if (!numberConvert) {
                qWarning("%s",
                         "Failed to convert filse size format unit system into "
        ...
        if (numArgs > 1) {
            ...
            if (!numberConvert) {
                qWarning("%s",
                         "Failed to convert file size format decimal precision "
        ...
        if (numArgs > 2) {
            ...
            if (!numberConvert) {
                qWarning("%s",
                         "Failed to convert file size format multiplier into "
                         "double value. Falling back to default 1.0");
                multiplier = 1.0F;
            } else {
                if (multiplier == 0.0F) {
                    qWarning("%s",
                             "It makes no sense to multiply the file size by zero. "
...
QVariant TruncateCharsFilter::doFilter(const QVariant &input, const QVariant &argument, bool autoescape) const
{
    Q_UNUSED(autoescape)
    QString retString = getSafeString(input);
