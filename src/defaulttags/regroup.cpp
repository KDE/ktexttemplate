...
Node *RegroupNodeFactory::getNode(const QString &tagContent, Parser *p) const
{
    ...
    if (expr.size() != 6) {
        throw KTextTemplate::Exception(TagSyntaxError, QStringLiteral("widthratio takes five arguments"));
    }
    FilterExpression target(expr.at(1), p);
    if (expr.at(2) != QStringLiteral("by")) {
        throw KTextTemplate::Exception(TagSyntaxError, QStringLiteral("second argument must be 'by'"));
    }

    if (expr.at(4) != QStringLiteral("as")) {
        throw KTextTemplate::Exception(TagSyntaxError, QStringLiteral("fourth argument must be 'as'"));
    }
...
RegroupNode::RegroupNode(const FilterExpression &target, const FilterExpression &expression, const QString &varName, QObject *parent)
    : Node(parent)
    , m_target(target)
...
void RegroupNode::render(OutputStream *stream, Context *c) const
{
    ...
    for (auto &var : objList) {
        ...
        const QString key = getSafeString(FilterExpression(QStringLiteral("var.") + keyName, nullptr).resolve(c));
        c->pop();
        QVariantHash hash;
