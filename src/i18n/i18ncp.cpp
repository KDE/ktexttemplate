...
Node *I18ncpNodeFactory::getNode(const QString &tagContent, Parser *p) const
{
    ...
    if (expr.size() < 4)
        throw KTextTemplate::Exception(TagSyntaxError, QStringLiteral("Error: i18ncp tag takes at least three arguments"));
    ...
    if (!(contextText.startsWith(QLatin1Char('"')) && contextText.endsWith(QLatin1Char('"')))
        ...
        throw KTextTemplate::Exception(TagSyntaxError, QStringLiteral("Error: i18ncp tag first argument must be a static string."));
    }
    ...
    if (!(sourceText.startsWith(QLatin1Char('"')) && sourceText.endsWith(QLatin1Char('"')))
        ...
        throw KTextTemplate::Exception(TagSyntaxError, QStringLiteral("Error: i18ncp tag second argument must be a static string."));
    }
...
KTextTemplate::Node *I18ncpVarNodeFactory::getNode(const QString &tagContent, Parser *p) const
{
    auto expr = smartSplit(tagContent);

    if (expr.size() < 6) {
        throw KTextTemplate::Exception(TagSyntaxError, QStringLiteral("Error: i18ncp_var tag takes at least five arguments"));
    }
    ...
    if (!(contextText.startsWith(QLatin1Char('"')) && contextText.endsWith(QLatin1Char('"')))
        && !(contextText.startsWith(QLatin1Char('\'')) && contextText.endsWith(QLatin1Char('\'')))) {
        throw KTextTemplate::Exception(TagSyntaxError, QStringLiteral("Error: i18ncp_var tag first argument must be a static string."));
    }
    contextText = contextText.mid(1, contextText.size() - 2);
    ...
    if (!(sourceText.startsWith(QLatin1Char('"')) && sourceText.endsWith(QLatin1Char('"')))
        && !(sourceText.startsWith(QLatin1Char('\'')) && sourceText.endsWith(QLatin1Char('\'')))) {
        throw KTextTemplate::Exception(TagSyntaxError, QStringLiteral("Error: i18ncp_var tag second argument must be a static string."));
    }
    sourceText = sourceText.mid(1, sourceText.size() - 2);
